...
cur.execute("SELECT m.name as 'Movie', m.year AS 'Year', d.last_name AS 'Director', m.ranking as 'Rank' FROM (movies_directors AS md JOIN movies as m on m.id = md.movie_id) JOIN directors AS d ON d.id = director_id WHERE m.ranking > 8 ORDER BY m.ranking DESC;")
rows = cur.fetchall()
print("")
print("== Consulta 3.3 ==")
for d in rows:
